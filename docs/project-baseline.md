...
The 58 doctest cases include 12 native/deep cases, eight CLI cases, three GUI presentation cases, two process-control identity cases, the original 18 formatting/settings/CPU cases and
...
| Release artifact checksums | Generated by tag workflow |
...
**`release.yml`** triggers on `v*.*.*` tag pushes. It validates the tag against
the canonical CMake version, builds/tests Release, packages the executable and
license as a versioned ZIP, generates a SHA-256 sidecar, and uploads both.
...
| Semantic versioning | Canonical CMake version `0.1.0` |
...
versioned usage, documentation links, and license. Architecture, security,
troubleshooting, and contribution details are in the linked documents.
...
| Risk | Severity | Notes |
|------|----------|-------|
| Version metadata drift | Low | Release workflow validates source, CLI, resource, and package versions |
