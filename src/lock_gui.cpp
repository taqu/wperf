...
namespace
{
    ...
    void ShowResult(HWND window, const LockGuiPresentation& view)
    {
        HWND list = GetDlgItem(window, ResultList);
        ...
        SetDlgItemTextW(window, StatusLabel, view.status.c_str());
    }
    ...
    LRESULT CALLBACK WindowProcedure(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
    {
        ...
        case WM_CREATE: {
            ...
            const int controls[] = {4199, PathEdit, BrowseFile, BrowseFolder, Inspect, DeepScan, 4198,
                                    4197, ResultList, StatusLabel, Refresh, Close};
...
int RunLockInspectorGui(HINSTANCE instance, int showCommand, std::wstring_view initialPath)
{
    ...
    while(!quit && (state->window.load() != nullptr || state->scanning)) {
        if(state->scanning) {
            const DWORD wait = MsgWaitForMultipleObjects(1, &state->completionEvent, FALSE, INFINITE, QS_ALLINPUT);
            if(wait == WAIT_OBJECT_0) {
                ...
                if(HWND active = state->window.load()) {
                    ShowResult(active, PresentLockInspection(completion->result));
