...
void ResourceMonitor::UpdateDiskMetrics()
{
    ...
    if(m_hDiskWriteCounter && PdhGetFormattedCounterValue((PDH_HCOUNTER)m_hDiskWriteCounter, PDH_FMT_DOUBLE, nullptr, &val) == ERROR_SUCCESS)
                ...
                if(needed > 0) {
                    ...
                    if(m_gpuBuf) {
                        ...
                        if(PdhGetFormattedCounterArrayW((PDH_HCOUNTER)hCounter,
                                                        PDH_FMT_DOUBLE, &needed, &count, pItems)
                           == ERROR_SUCCESS) {
                            double total = 0.0;
